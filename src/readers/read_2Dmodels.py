...
def RequestData():
    ...
    if Depth < 0:
       X,Y,Z,V,meta = lib.read2DnetCDFFile(address,Latitude_Variable,Longitude_Variable,Variable,(Latitude_Begin,Longitude_Begin),(Latitude_End,Longitude_End),inc=Sampling)
    else:
       X,Y,Z,V,meta = lib.read2DnetCDFFile(address,Latitude_Variable,Longitude_Variable,Variable,(Latitude_Begin,Longitude_Begin),(Latitude_End,Longitude_End),inc=Sampling,setDepth=Depth) 
    nx = len(X)
    if nx <= 0:
    ...
    Label2 = " - %s (lat:%0.1f,%0.1f, lon:%0.1f,%0.1f)"%(lib.areaValues[Area],meta['lat'][0],meta['lat'][1],meta['lon'][0],meta['lon'][1])
